...
def build_notebook() -> dict:
    cells: list[dict] = [
        markdown_cell(
            "# Experiment: SpectralBio Block 10B - State-Matched Structural Dissociation in TP53 (H100)\n\n"
            "Objective:\n"
            "- Repair the weakest point of Block 10: WT reference selection that can inflate global RMSD and blur local mechanistic interpretation.\n"
            "- Use state-matched TP53 experimental structures as an external ground truth for local geometric perturbation.\n"
            "- Evaluate multiple WT references per mutant, choose the best structural control by minimum global RMSD, and keep a strict mechanistic subset.\n"
            "- Recompute covariance-aware SpectralBio scores directly on the repaired structural subset.\n"
            "- Test whether covariance-derived hidden-state geometry tracks local or excess-local structural tension better than scalar `ll_proper`.\n"
            "- Save a complete Lightning-friendly bundle under `New Notebooks/results/07b_block10_structural_dissociation_tp53_h100/`.\n"
        ),
        markdown_cell(
            "## Block 10B Deliverables\n\n"
            ...
            "- Broad and strict structural analyses with global alignment, local window RMSD, excess-local RMSD, and local contact-change summaries.\n"
            "- Fresh ESM2-150M scoring on the exact structural subset: `ll_proper`, `frob_dist`, `trace_ratio`, `sps_log`, and pair score.\n"
            "- A predeclared covariance-family metric sweep across `frob_dist`, `trace_ratio`, and `sps_log`, reported alongside `ll_proper`.\n"
            "- A blinded dissociation analysis on the strict subset when enough clean pairs survive, otherwise on the repaired broad subset.\n"
            ...
            "- All outputs are written under `New Notebooks/results/07b_block10_structural_dissociation_tp53_h100/`.\n"
        ),
        code_cell(
            dedent(
                """
                ...
                NOTEBOOK_SLUG = '07b_block10_structural_dissociation_tp53_h100'
                ...
                TOP_REFERENCE_CANDIDATES = int(os.environ.get('SPECTRALBIO_BLOCK10_TOP_REFERENCE_CANDIDATES', '12'))
        ...
        code_cell(
            dedent(
                ...
                if metric_sweep.empty:
                    raise RuntimeError('No covariance metric sweep rows were produced for the Block 10B analysis frame.')
        ...
        code_cell(
            dedent(
                ...
                group_c_count = int((analysis_variant_level['blinded_group'] == 'group_c_low_geometry_low_likelihood').sum())

                claim_status = 'weak_tp53_state_matched_structural_dissociation'
                claim_reason = 'The repaired TP53 structural subset is informative, but the state-matched evidence is not yet strong enough for a mechanistic claim.'
                if (
                    ...
                    and best_cov_abs_spearman >= 0.45
                    and analysis_abs_gap >= 0.20
                    and group_a_count >= 1
                ):
                    claim_status = 'tp53_state_matched_structural_dissociation_supported'
                    claim_reason = 'After state-matched WT repair, a covariance-derived geometry metric tracks the strict TP53 structural signal substantially better than scalar likelihood.'
                elif (
                    analysis_level_name == 'strict_variant_level'
                    and len(analysis_variant_level) >= 3
                    and not math.isnan(best_cov_abs_spearman)
                    and not math.isnan(ll_abs_target_spearman)
                    and best_cov_abs_spearman > ll_abs_target_spearman
                ):
                    claim_status = 'bounded_tp53_state_matched_case_study_supported'
                    claim_reason = 'The strict state-matched TP53 subset favors covariance-derived geometry over scalar likelihood, but the safest framing is still a bounded mechanistic case study.'
                elif group_a_count >= 1 and not math.isnan(best_cov_abs_spearman):
                    claim_status = 'case_level_tp53_repair_supported'
                    claim_reason = 'The strongest support is case-level: at least one repaired TP53 pair shows high structural disruption with only modest scalar likelihood penalty.'
                ...
                claim_paragraph = (
                    'Using experimental TP53 crystal structures as an external geometric ground truth, Block 10B repairs WT reference selection before making any mechanistic claim. '
                    ...
                    f'The primary analysis runs on `{analysis_level_name}` using `{analysis_target_label}` as the structural target. '
                    ...
                    'This still needs to be described with care, but it directly addresses the reviewer question of whether covariance is merely a nonlinear rewrite of likelihood: after repairing structural controls, the covariance side remains more tightly coupled to structural tension than the scalar sequence-surprise score.'
                )
                ...
                summary_md = '\\n'.join([
                    '# Block 10B Structural Dissociation Summary',
                    '',
                ...
                print(summary_md)
                done('Block 10B structural dissociation bundle is ready for download.')
                """
            )
        ),
    ]
...
def main() -> None:
    ...
    output_path = repo_root / "New Notebooks" / "07b_block10_structural_dissociation_tp53_h100.ipynb"
