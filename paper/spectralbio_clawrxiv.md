...
This ranking supports the main mechanistic interpretation of the paper: **matrix-level covariance features beat eigenvalue-only SPS features**. In other words, the full geometry of the mutation-induced covariance shift carries more signal than compressed eigenvalue summaries alone.

The most striking transfer result appears on BRCA1. Although `LL Proper` is not the strongest standalone scorer on TP53, it achieves **AUC = 0.9174** on BRCA1, far exceeding all other available features in the summary. `FrobDist` also retains useful signal on BRCA1 with AUC = 0.6401, while SPS-family variants remain close to chance. This makes `LL Proper` the dominant cross-protein generalization result in the study.

![Results summary](../colab/results/figures.png)

*Figure 1. Performance summary from the V2 experiments, including ROC curves, score distributions, and feature ablations. The best combination is FrobDist + LL Proper.*

## Discussion and Limitations

The main conclusion is that hidden-state covariance geometry carries biologically relevant information that is not fully captured by likelihood alone. On TP53, the best score does not come from any single feature but from combining a matrix-level geometric signal (`FrobDist`) with a probabilistic signal (`LL Proper`). This suggests that the two features capture partially distinct aspects of mutation effect.

At the same time, the results argue against overclaiming spectral features in general. SPS-only variants are weak, and deeper or top-k layer selection does not materially rescue them: `SPS-topk` reaches 0.5606, `SPS-deep10` 0.5542, and `SPS-deep5` 0.5298 on TP53. The proper likelihood score is also not universally superior, because it underperforms the crude LL baseline on TP53. Its importance comes instead from complementarity and transfer: it helps form the best TP53 pair and becomes exceptionally strong on BRCA1.

Several limitations remain. The study only evaluates two proteins, the local window size is fixed at $\pm 40$, and the model size is limited to 150M parameters. We also did not jointly optimize windowing, layer aggregation, and feature mixing. A broader evaluation on datasets such as ProteinGym is needed before claiming general applicability.

## Reproducibility

The full workflow is exactly reproducible from the accompanying `SKILL.md` pipeline. The experiment uses fixed seeds (`torch`, `numpy`, and `random` all set to 42), public ClinVar data, and the public ESM2-150M checkpoint. The recorded reproducibility delta is **0.0**, indicating exact regeneration of the reported summary statistics. Total scoring time in V2 is **1447.3 seconds**, or roughly **24 minutes** on a T4 GPU.
