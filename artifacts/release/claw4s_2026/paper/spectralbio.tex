...
\usepackage[
  ...
  pdftitle={SpectralBio: Full-Matrix Covariance Analysis for Zero-Shot Variant Pathogenicity on the TP53 Canonical Benchmark},
  pdfauthor={Davi Bonetto; Claw4S AI Co-author},
  pdfsubject={Claw4S Conference 2026 four-page paper},
  pdfkeywords={SpectralBio, TP53, protein language model, covariance, zero-shot variant pathogenicity, ESM2, reproducibility}
]{hyperref}
...
% --- Title ---
\begin{center}
{\LARGE\bfseries \methodname{}: Full-Matrix Covariance Analysis\\[3pt]
for Zero-Shot Variant Pathogenicity on the\\[3pt]
TP53 Canonical Benchmark\par}
\vspace{6pt}
%
% --- Authors ---
{\large
...
A supplementary exact-split ESM-1v ensemble reaches \textbf{AUC\,=\,0.9465} on TP53,
so the contribution is best read as a covariance-centered representational
benchmark artifact rather than a raw-AUC state-of-the-art claim.
...
\textbf{What the exact-split external comparison changes.}\;
A supplementary exact-split ESM-1v five-model ensemble on the same TP53 variants
reaches AUC=0.9465, exceeding \methodname{} (0.7498).
This sharpens the claim boundary: the contribution here is not external raw-AUC
state of the art, but covariance-based representational analysis plus a
reproducible executable benchmark artifact.
...
\begin{enumerate}[itemsep=1pt]
  ...
  \item \textbf{External context and descriptive phase-5 geometry.}\;
    Exact-split ESM-1v is stronger on TP53, while supplementary Grantham bins
    localize relative \methodname{} strength to more chemically disruptive substitutions
    and a residual covariance-likelihood UMAP shows partial label organization only.
...
Supplementary phase-5 analyses remain descriptive rather than confirmatory:
Grantham stratification localizes relative performance, and the residual
covariance-likelihood UMAP shows only partial class organization
(silhouette = 0.133).
...
At the same time, a supplementary exact-split ESM-1v ensemble reaches AUC=0.9465
on TP53, so \methodname{} is best interpreted as a bounded representational
result and reproducibility artifact rather than as the strongest raw-AUC
external predictor on this split.
These results are therefore presented as a research reproducibility artifact
with one primary benchmark, one bounded secondary transfer evaluation, and
descriptive supplementary geometry analyses rather than broad generalization claims.
